    ...
    try:
        ...
        class AsynCoroSocket(_AsynCoroSocket):
            ...
            def _iocp_recvall(self, bufsize, *args):
                ...
                def _recvall(self, view, err, n):
                    if err or n == 0:
                        ...
                        if len(view) == 0:
                            ...
                            err, n = win32file.WSARecv(self._fileno, view, self._overlap, 0)
                            if err and err != winerror.ERROR_IO_PENDING:
                                if self._timeout and self._notifier:
                                    self._notifier._del_timeout(self)
                                view.release()
                ...
                view = self._result
                self._overlap.object = functools.partial(_recvall, self, view)
                self._coro = self._asyncoro.cur_coro()
                self._coro._await_()
                if self._timeout:
                    self._notifier._add_timeout(self)
                    ...
                    def _ssl_handshake(self, err, n):
                        try:
                            ...
                            if err.args[0] == ssl.SSL_ERROR_WANT_READ:
                                ...
                                err, n = win32file.WSASend(self._fileno, b'', self._overlap, 0)
                            else:
                                raise socket.error(err)
                        except:
                            if self._timeout and self._notifier:
                    ...
                    def _ssl_handshake(self, conn, addr, err, n):
                        try:
                            ...
                            if err.args[0] == ssl.SSL_ERROR_WANT_READ:
                                ...
                                err, n = win32file.WSASend(conn._fileno, b'', self._overlap, 0)
                            else:
                                raise socket.error(err)
                        except:
                            if self._timeout and self._notifier:
